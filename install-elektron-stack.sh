...
FAUCET_FEE_BUMP_INTERVAL_MIN="10"             # minutes between checks that RBF fee-bump any faucet payout still unconfirmed because the node's live mempoolminfee rose above what it paid; also editable later in Admin Panel -> Mempool & Fee Bumping, no restart needed
...
# add one if whoever set it (config file or the prompt above) left it as a
# bare domain.
if [ -n "$POOL_URL" ]; then
  case "$POOL_URL" in
    http://*|https://*) ;;
...
FAUCET_FEE_BUMP_INTERVAL_MIN=${FAUCET_FEE_BUMP_INTERVAL_MIN}
ENV_EOF
fi
